...
describe("AccessPolicySelection", () => {
  test("dont show passphrase input on default state", () => {
    const {container} = render(<AccessPolicySelection accessPolicy={AccessPolicy.PUBLIC} onAccessPolicyChange={vi.fn()} passphrase="" onPassphraseChange={vi.fn()} />);
  ...
  test("show passphrase input on access policy by passphrase", () => {
    const {container} = render(<AccessPolicySelection accessPolicy={AccessPolicy.BY_PASSPHRASE} onAccessPolicyChange={vi.fn()} passphrase="" onPassphraseChange={vi.fn()} />);
    expect(container.querySelector(`[data-testid="passphrase-input"]`)).toBeDefined();
  });

  test("trigger on change on passphrase change", async () => {
    const onChangeOfPassphrase = vi.fn();
    const {container} = render(
      <AccessPolicySelection accessPolicy={AccessPolicy.BY_PASSPHRASE} onAccessPolicyChange={vi.fn()} passphrase="" onPassphraseChange={onChangeOfPassphrase} />
    );
